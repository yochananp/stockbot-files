...
MAX_TICKERS    = 50   # limit to avoid long runtime
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(r"C:\Users\yocha\stockbot\options_sheets.log"),
    ]
)
...
def write_to_sheet(service, sheet_id: str, tab: str, data: list[list]):
    """Write data (list of rows) to a sheet tab."""
    body = {"values": data}
...
def format_sheet(service, sheet_id: str, sheet_gid: int, num_rows: int, num_cols: int):
...
def calc_greeks(S, K, T, r, sigma, opt_type="call") -> dict:
...
def calc_iv_rank(hist_iv, current_iv) -> Optional[float]:
...
def recommend_strategy(alert: str, iv_rank: Optional[float]) -> tuple[str, str]:
    ...
    return strat, f"IV Rank {iv_rank:.0f} — low IV, buy premium" if iv_rank else f"{strat} — IV unavailable"


def analyze_ticker(ticker: str, alert: str = "NEUTRAL") -> Optional[dict]:
    try:
        ...
        # IV current (ATM from nearest expiry)
        iv_current = None
        try:
        ...
        try:
            ...
            iv_rank  = calc_iv_rank(roll_vol.dropna().tolist(), iv_current)
        except Exception:
            pass
        ...
        # Best contract per expiry
        best_contracts = []
        for exp in filtered[:4]:  # max 4 expirations
            dte = _days_to_expiry(exp)
            T   = dte / 365.0
            try:
...
def main():
    log.info("=" * 60)
    log.info("Options → Google Sheets — starting")
    log.info("=" * 60)
    ...
    for i, ticker in enumerate(tickers):
        ...
        result = analyze_ticker(ticker, alert="NEUTRAL")
        if not result:
    ...
    # Summary tab
    summary_header = [
        "Date", "Ticker", "Alert", "Spot", "IV%", "IV Rank",
        "Strategy", "Rationale"
    ]
    ...
    # Contracts tab
    contracts_header = [
        "Date", "Ticker", "Strategy", "Expiry", "DTE", "Strike", "Type",
    ...
    for sheet in sheets_meta["sheets"]:
        ...
        rows  = len(summary_rows) + 1 if title == "Summary" else len(contract_rows) + 1
        cols  = len(summary_header) if title == "Summary" else len(contracts_header)
        format_sheet(service, sheet_id, gid, rows, cols)
